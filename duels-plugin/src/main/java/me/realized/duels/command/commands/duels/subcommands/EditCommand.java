...
public class EditCommand extends BaseCommand {
    ...
    public EditCommand(final DuelsPlugin plugin) {
        super(plugin, "edit", "edit [name] [add:remove:set] [wins:losses] [amount]",
                "Edits player's wins or losses.", 5, false);
    ...
    protected void execute(final CommandSender sender, final String label, final String[] args) {
        ...
        if (action == null) {
            lang.sendMessage(sender, "ERROR.command.invalid-action", "action",
                    actionName, "available_actions", actions.keySet());
            return;
        }
        ...
        if (setter == null) {
            lang.sendMessage(sender, "ERROR.command.invalid-option", "option",
                    option, "available_options", getters.keySet());
            return;
        }

        final int amount = Math.max(NumberUtil.parseInt(args[4]).orElse(0), 0);
        setter.accept(user, action.apply(user, option, amount));
        lang.sendMessage(sender, "COMMAND.duels.edit", "name", user.getName(),
                "type", option, "action", actionName, "amount", amount);
    }
